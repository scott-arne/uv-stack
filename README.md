...
inside `[tool.uv-stack]` that means the `stack` key, since the rest of that
table is uv-stack's own bookkeeping. `requirements.in` and `environment.yml`
are generated and carry a `# Generated by uv-stack — do not edit.` header,
and `uv` compiles `requirements.lock.txt` from `requirements.in`. On the
project path `stack` writes the resolved packages into your own
`pyproject.toml` and records what it applied back into `[tool.uv-stack]`,
leaving the lock and `.venv` to `uv`.
...
value that names an existing file is used as-is, so a path containing spaces
needs no quoting. The edited file is appended as the final argument. An empty
